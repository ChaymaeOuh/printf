	...
	int flags, int width, int precision, int size)
{
	...
	if (width > 1)
	{
		...
		if (flags & FLAG_MINUS)
			...
			write(1, &buffer[BUFFER_SIZE - i - 1],
				width - 1));
		else
...
/**
 * my_write_number - Prints a string
 ...
 * Return: Number of chars printed.
 */
int my_write_number(int is_negative, int ind, char buffer[],
	int flags, int width, int precision, int size)
{
	...
	return (my_write_num(ind, buffer, flags, width, precision,
...
/**
 * my_write_unsgnd - Writes an unsigned number
...
/**
 * my_write_pointer - Write a memory address
